...
export default function HomeClient() {
  ...
  const { basketIds, toggleBasket } = useFavorites();
  ...
  const [toastImageId, setToastImageId] = useState<number | null>(null);
  ...
  const applyPreviewState = useCallback((imageId: number | null) => {
  ...
  const syncPreviewUrl = useCallback((imageId: number | null, replace: boolean) => {
    if (typeof window === "undefined") return;
    const url = new URL(window.location.href);
    if (imageId === null) {
      url.searchParams.delete("piece");
    } else {
      url.searchParams.set("piece", String(imageId));
    }
  ...
  const openPreviewOverlay = useCallback((imageId: number) => {
    applyPreviewState(imageId);
    syncPreviewUrl(imageId, false);
  }, [applyPreviewState, syncPreviewUrl]);

  const closePreviewOverlay = useCallback(() => {
    applyPreviewState(null);
    syncPreviewUrl(null, true);
  }, [applyPreviewState, syncPreviewUrl]);
  ...
  const navigatePreview = useCallback((direction: "left" | "right" | "up" | "down") => {
    ...
    if (newId > 0 && newId <= TILE_COUNT) {
      ...
      syncPreviewUrl(newId, true);
    }
  }, [previewImageId, syncPreviewUrl]);
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(id) && id >= 1 && id <= TILE_COUNT) {
      applyPreviewState(id);
      setGetByIdInput(String(id));
    }
  }, [applyPreviewState]);
    ...
    return (
      ...
      >
        ...
        <div
          ...
          onClick={(e) => { e.stopPropagation(); const was = basketIds.has(tile.id); toggleBasket(tile.id); setToastImageId(tile.id); setToastAction(was ? "removed" : "added"); }}
        >
  ...
  }, [handleTileClick, basketIds, toggleBasket]);
  ...
  // Apply detected language after hydration to avoid SSR mismatch
  ...
  return (
    <>
      ...
      <div id="fullpage" ref={fullpageRef}>
        ...
        {/* --- Section 9: Sofa --- */}
        <SofaSection palettesByImageId={palettesByImageId} />

        {/* --- Section 10: Production / Delivery explainer --- */}
        <ProductionSection
          firstPriceFormatted={firstPriceFormatted}
          lastPriceFormatted={lastPriceFormatted}
          totalPiecesCopy={TOTAL_PIECES_COPY}
        />

        {/* --- Section 11: Bid --- */}
        <div id="bid-slide" style={{ display: "none" }}>
        ...
        >
          ...
          <div className="overlay-actions" onClick={(e) => e.stopPropagation()}>
            <div
              ...
              onClick={() => { const was = basketIds.has(overlayPreviewTile.id); toggleBasket(overlayPreviewTile.id); setToastImageId(overlayPreviewTile.id); setToastAction(was ? "removed" : "added"); }}
            >
      ...
      <Toast
        imageId={toastImageId}
        action={toastAction}
        onDismiss={() => setToastImageId(null)}
      />
    </>
  );
}
